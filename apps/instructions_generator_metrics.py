...
class InstructionsGeneratorMetrics(object):
  ...
  def flush(self, epoch, idx, train=True):
    if train:
      print(
          '[%d, %5d] train loss: %.3f, train bleu: %.3f, train token acc: %.3f'
          % (epoch + 1, idx + 1, self.get_mean(
              self.running_losses), self.get_mean(self.running_bleu_scores),
             self.get_mean(self.running_token_accuracy)))
    else:
      print(
          '[%d, %5d] valid loss: %.3f, valid bleu: %.3f, valid token acc: %.3f'
          % (epoch + 1, idx + 1, self.get_mean(
              self.running_losses), self.get_mean(self.running_bleu_scores),
             self.get_mean(self.running_token_accuracy)))
