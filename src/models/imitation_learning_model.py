...
class ImitationLearningModel(nn.Module):
	"""docstring for ImitationLearningModel"""
	def __init__(self, config):
		super(ImitationLearningModel, self).__init__()
		self.config = config
		# linear layer to encode grid embedding
		self.fc_grid_emb = nn.Linear(config.embedding_dim, config.grid_embedding_hidden_size)
		# linear layer to encode grid onehot
		self.fc_grid_onehot = nn.Linear(config.state_onehot_size, config.grid_onehot_hidden_size)
		# linear layer to encode grid
		self.fc_grid = nn.Linear(config.grid_embedding_hidden_size + config.grid_onehot_hidden_size, config.grid_hidden_size)
		# linear layer to encode goal embedding
		self.fc_goal_emb = nn.Linear(config.embedding_dim, config.goal_embedding_hidden_size)
		# linear layer to encode grid and goal
		self.fc_grid_goal = nn.Linear(
			config.state_size * config.grid_hidden_size + config.goal_embedding_hidden_size, config.grid_goal_hidden_size)
		# linear layer to encode inventory
		self.fc_inventory = nn.Linear(config.embedding_dim, config.inventory_hidden_size)
		# linear layer to encode everything
		self.fc_all = nn.Linear(config.grid_goal_hidden_size + config.inventory_hidden_size, config.all_hidden_size)
		# projection layers
	...
	def forward(self, grid_embedding, grid_onehot, inventory_embedding, goal_embedding):
		...
		x_grid_1 = x_grid_1.view(-1, self.config.state_size, self.config.grid_embedding_hidden_size)
		...
		x_grid_2 = x_grid_2.view(-1, self.config.state_size, self.config.grid_onehot_hidden_size)
		...
		x_grid = x_grid.view(-1, self.config.state_size*self.config.grid_hidden_size)
		...
		x = F.relu(self.fc_all(torch.cat((x_grid_goal, x_inventory), dim=1)))
