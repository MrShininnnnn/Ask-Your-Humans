...
def play(model, glove, config):
    ...
    while not game.is_over() and count < 250:
        ...
        states_embedding = torch.from_numpy(np.array([dataloader.get_grid_embedding(state, glove, config.embedding_dim)]))
        states_onehot = torch.from_numpy(np.array([dataloader.one_hot_grid(state, glove, config.embedding_dim)]))
        goal = torch.from_numpy(dataloader.get_goal_embedding(goal, glove, config.embedding_dim))
        inventory = torch.Tensor(dataloader.get_inventory_embedding(inventory, glove, config.embedding_dim))
